...
// Start lookup
	...
	for (int i = 0; i < BUCKET_SIZE; i++) {
		...
		char res = append_event_to_queue(
...
__device__ // private
void reverse_event_type_2(Event *event) {
	uint nid = event->receiver;
	int event_timestamp = abs(event->timestamp);

	curandState_t *cr_state = &(nodes.cr_state[nid]);

	uint lookup_pid = nodes.lookup_pid[nid];
	uint bucket_id = get_bucket_id(nid, lookup_pid);
	uint *bucket = get_bucket(nid, bucket_id);
	...
	for (int i = BUCKET_SIZE - 1; i >= 0; i --) {
		...
		reverse_state(cr_state);
		curandState_t cr_state_old = *cr_state;

		Event antimsg;
		antimsg.type = 4;
		antimsg.sender = nid;
		antimsg.receiver = receiver_nid;
		antimsg.timestamp = event_timestamp +
			get_latency(&cr_state_old);
	...
	nodes.n_pending_requests[nid] = 0;
...
__device__ // private
void reverse_event_type_3(Event *event) {
	uint nid = event->receiver;
	int event_timestamp = abs(event->timestamp);
	...
	if (nodes.n_pending_requests[nid] == 0) {
		...
		curandState_t cr_state_old = *cr_state;
		int lookup_delay =
			random(&cr_state_old, max_lookup_delay + 1) + 1;

		Event antimsg;
		antimsg.type = 2;
		antimsg.sender = nid;
		antimsg.receiver = nid;
		antimsg.timestamp = event_timestamp + lookup_delay;
		...
		for (int i = BUCKET_SIZE - 1; i >= 0; i--) {
			if (lookup_results[i].status != 3) { continue; }
			...
			antimsg.type = 4;
			antimsg.sender = nid;
			antimsg.receiver = lookup_results[i].nid;
			antimsg.timestamp =
				event_timestamp + get_latency(&cr_state_old);
...
__device__ // private
void reverse_event_type_4(Event *event) {
	uint nid = event->receiver;
	...
	antimsg.type = 3;
	antimsg.sender = nid;
	antimsg.receiver = event->sender;
	antimsg.timestamp = abs(event->timestamp) + get_latency(&cr_state_old);
...
void roll_back_event(Event *event) {
	...
	if (type == 2) {
		reverse_event_type_2(event);
	} else if (type == 3) {
		reverse_event_type_3(event);
	} else if (type == 4) {
		reverse_event_type_4(event);
	} else {
